...
function init() {
    ...
    makeScrollBar();
    setTimeout(() => {
        getPages()[currentPage].classList.add("active");
    }, 50);
...
function setPageSize() {
    ...
    getPages().forEach(function (e) {
        e.style.width = "100vw";
        e.style.height = (100 * unit.vh()) + "px";
        e.style.position = "absolute";
        e.style.top = (i * (100 * unit.vh())) + "px";
...
function scroll() {
    ...
    updateScrollBar();
    getPages().forEach(function (e) {
        e.style.transform = "translateY(-" + currentPage * (100 * unit.vh()) + "px)";
    });
    setTimeout(function () {
        midAnimation = false;
    }, 800);
...
function updateScrollBar() {
    ...
    select(".page-number-box").innerHTML = (currentPage + 1) < 10 ? "0" + (currentPage + 1) : (currentPage + 1);
}
